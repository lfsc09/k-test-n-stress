...
func (m *Mock) CobraList(out io.Writer) {
	...
	fmt.Fprintf(out, "%s\n", tableLineData(colSizes, []string{"Boolean.booleanWithChance:{probability}", "Generates a random boolean with a chance of true"}))
	fmt.Fprintf(out, "%s\n", tableLineDivider(colSizes))
	...
	fmt.Fprintf(out, "%s\n", tableLineDivider(colSizes))
	fmt.Fprintf(out, "%s\n", tableLineData(colSizes, []string{"Lorem.paragraph:{number}", "Generates a random paragraph with N number of sentences"}))
	fmt.Fprintf(out, "%s\n", tableLineData(colSizes, []string{"Lorem.paragraphs:{number}", "Generates N number of random paragraphs"}))
	fmt.Fprintf(out, "%s\n", tableLineData(colSizes, []string{"Lorem.sentence:{number}", "Generates a random sentence with N number of words"}))
	fmt.Fprintf(out, "%s\n", tableLineData(colSizes, []string{"Lorem.sentences:{number}", "Generates N number of random sentences"}))
	fmt.Fprintf(out, "%s\n", tableLineData(colSizes, []string{"Lorem.word", "Generates a random word"}))
	fmt.Fprintf(out, "%s\n", tableLineData(colSizes, []string{"Lorem.words:{number}", "Generates N number of random words"}))
	fmt.Fprintf(out, "%s\n", tableLineDivider(colSizes))
